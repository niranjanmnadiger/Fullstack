...
export default function NammaMetroTripPlanner() {
  ...
  const [holidaysInput, setHolidaysInput] = useState("2026-01-26\n2026-08-15\n2026-10-02");
  const [result, setResult] = useState(null);

  const now = useMemo(() => new Date(), []);
  const holidaySet = useMemo(
    () => new Set(holidaysInput.split(/\s+/).map((x) => x.trim()).filter(Boolean)),
    [holidaysInput]
  );
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-neutral-200">
          ...
          <p className="mt-2 text-sm text-neutral-600">
            React starter for next-train estimation with line changes at Majestic and RV Road.
          </p>
        ...
        <SectionCard title="Holiday Overrides">
          <p className="mb-3 text-sm text-neutral-600">
            Put one holiday per line in <code>YYYY-MM-DD</code>. You can later fetch these from an API.
          </p>
          <textarea
            className="min-h-32 w-full rounded-2xl border border-neutral-300 p-3 outline-none focus:ring-2 focus:ring-violet-400"
            value={holidaysInput}
